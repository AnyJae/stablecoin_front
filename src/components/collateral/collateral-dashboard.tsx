...
import { TrendingUp, AlertTriangle, Shield, DollarSign, BarChart3 } from 'lucide-react';
...
export function CollateralDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'assets' | 'alerts' | 'analytics'>('overview');
        ...
        <div className="bg-ksc-gray-dark border border-ksc-gray p-3 rounded-lg">
          ...
          {payload.map((entry: any, index: number) => (
            <p key={index} className="text-sm" style={{ color: entry.color }}>
              {`${entry.name}: ${entry.value}${entry.name === 'collateralRatio' ? '%' : '억원'}`}
            </p>
          ))}
        </div>
      );
  ...
  return (
    <div className="space-y-8">
      <div className="flex space-x-1 bg-ksc-gray-dark rounded-lg p-1">
        ...
        >
          <BarChart3 size={20} />
          <span>{t('collateral.tabs.overview')}</span>
        </button>
        ...
        >
          <DollarSign size={20} />
          <span>{t('collateral.tabs.assets')}</span>
        </button>
        ...
        >
          ...
          <span>{t('collateral.tabs.alerts')}</span>
        </button>
        ...
        >
          <TrendingUp size={20} />
          <span>{t('collateral.tabs.analytics')}</span>
        </button>
      </div>
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
            ...
            <div className="bg-ksc-gray-dark rounded-lg p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">{formatCurrency(collateralData.totalValue)}</p>
                </div>
            ...
            <div className="bg-ksc-gray-dark rounded-lg p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">{formatCurrency(collateralData.kscSupply)}</p>
                </div>
            ...
            <div className="bg-ksc-gray-dark rounded-lg p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">{formatRatio(collateralData.collateralRatio)}</p>
                </div>
            ...
            <div className="bg-ksc-gray-dark rounded-lg p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className={`text-2xl font-bold ${collateralData.isHealthy ? 'text-green-500' : 'text-red-500'}`}>
                    {collateralData.isHealthy ? '안정' : '위험'}
                  </p>
                </div>
                <div className={`p-2 rounded-full ${collateralData.isHealthy ? 'bg-green-500' : 'bg-red-500'}`}>
          ...
          <div className="bg-ksc-gray-dark rounded-lg p-6">
            <h3 className="text-xl font-semibold mb-4">담보 비율 추이 (최근 30일)</h3>
            <div className="h-64">
            ...
            {collateralData.assets.map((asset: any, index: number) => (
              <div key={index} className="bg-ksc-gray rounded-lg p-4">
                <div className="flex justify-between items-center mb-3">
                  <div>
                    <h4 className="font-semibold">{asset.name} ({asset.symbol})</h4>
                    <p className="text-sm text-ksc-gray-light">
                      {new Intl.NumberFormat('ko-KR').format(parseFloat(asset.amount))} {asset.symbol}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="font-semibold">{formatCurrency(asset.valueInKRW)}</p>
                    <p className="text-sm text-ksc-gray-light">{asset.ratio}%</p>
                  </div>
                </div>
            ...
            {[
              {
                ...
                type: 'warning',
                title: '담보 비율 하락',
                message: '현재 담보 비율이 115%로 최소 요구 비율(110%)에 근접하고 있습니다.',
                severity: 'medium',
              ...
              <div key={alert.id} className={`p-4 rounded-lg border-l-4 ${
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-sm text-ksc-gray-light mt-1">{alert.message}</p>
      ...
      {activeTab === 'analytics' && (
        <div className="space-y-6">
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      ...
                      dataKey="value"
                    >
                      {collateralData.pieChartData.map((entry: any, index: number) => (
          ...
          <div className="bg-ksc-gray-dark rounded-lg p-6">
            <h3 className="text-xl font-semibold mb-4">KSC 공급량 및 총 담보 가치 추이</h3>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={collateralData.chartData}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
